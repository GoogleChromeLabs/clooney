 ...
 */
export let defaultWorkerSrc: string =
  "document" in self
...
export class RoundRobinStrategy implements Strategy {
  ...
  private async _initOrGetContainer(i: number): Promise<ActorContainer> {
    if (i >= this._containers.length) throw Error("No worker available");
 ...
 */
export function addErrorHandlingToContainer(container: ActorContainer): void {
  const originalSpawn = container.spawn.bind(container);
  container.spawn = async (actor: ActorSource, opts: Object) => {
    try {
